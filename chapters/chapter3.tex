...
respectively. Given a receptive field (or patch) size of $w_p \times h_p$, the
image $X$ is split into a set of $I \times J$ (non-overlapping) patches
...
\begin{align}
    o^{\uparrow}_{i,j} = f^\uparrow(C^{\uparrow}_{i,j-1},p_{i,j}),
        &\text{ for }j=1,\cdots, J\\
    o^{\downarrow}_{i,j} = f^\downarrow(C^{\downarrow}_{i,j+1},p_{i,j}),
        &\text{ for }j=J,\cdots,1
\end{align}
...
$o^{\uparrow}_{i,j}$ and $o^{\downarrow}_{i,j}$ are concatenated at each
location $(i,j)$ to get a composite feature map
$O^\updownarrow=~\left\{o_{i,j}^{\uparrow},~o_{i,j}^{\downarrow}\right\}_
{i=1,\ldots,I}^{j=1,\ldots,J}$, where each element $o^\updownarrow_{i,j} \in
\RR^{2d}$ and $d$ is the number of recurrent units. Each $o_{i,j}$ is now the
activation of a feature detector at the location $(i,j)$ with respect to all
the patches in the $j$-th column of the original input ($p_{i, j}$ for all
$i$).

Next, the obtained composite feature map $O^\updownarrow$ is swept
...
of $O^\updownarrow$ and produce an output feature map $O^\leftrightarrow =
\left\{o^\leftrightarrow_{i,j} \right\}$, where $o^\leftrightarrow_{i,j} \in
\RR^{2d}$. In this representation, each vector $o^\leftrightarrow_{i,j}$
represents the features of the original image patch $p_{i,j}$
\emph{in the context of the whole image}.
...
map) $X$ of one ReNet layer to the output feature map $O^\leftrightarrow$
