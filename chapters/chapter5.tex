...
sequence lengths. Clearly the models has generalization issues on this dataset,
especially when the time correlation is taken into account. This is not too
surprising given the limited number of images in the CamVid dataset, as will be
discussed in more detail in~\autoref{sec:deconvLSTM_discussion}.

For completeness, \autoref{tbl:deconvLSTM_camvid} compares the results of the
DEConvLSTM model with the state-of-the-art on Camvid. Clearly there is a
margin for improvement, which can be probably gained reducing the
generalization issues of the model with stronger regularization and relying on
data augmentation to diminish the overfitting. Not too much effort was spent on
hyperparameters tuning on this dataset, as the main goal was to gain confidence
with the model rather than to reach the state-of-the-art.
...
%%%%%% DAVIS %%%%%%%%%%
\autoref{tbl:deconvLSTM_davis} reports the initial results of the
...
%%%%%%%%% GATECH %%%%%%%%%
Finally, \autoref{tbl:deconvLSTM_gatech} reports the results of the DEConvLSTM
model on the Gatech Dataset. Remarkably, the performance of the DEConvLSTM
model on this dataset aligns with the state-of-the-art. As discussed before,
many modifications (such as data augmentation) can be adopted to increase the
