...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[domain=-2.9:2.9]
        ...
        %    node[right] {$f(x) = logistic(x), beta=.5$};
        % Tanh
        ...
        \draw[thick, color=red]
            plot (\x, { max(0, \x) })
            %node[right] {$ReLU(x)$};
            node[xshift=-19.3em,yshift=-8em] (a) {$ReLU(x)$};
        % Leaky ReLU
        \draw[thick, color=yellow]
            plot (\x, { max(0.03 * \x, \x) })
            %node[above, xshift=-13.7em,yshift=-10.5em] {$Leaky ReLU(x)$};
            %node[above, xshift=-20em,yshift=-10.5em] {$Leaky ReLU(x)$};
            node[below = 1.2em of a.east, anchor=east] {$Leaky ReLU(x)$};
    \end{tikzpicture}
...
Note that it is possible to extend the softmax with a parameter $\beta$ that
controls its steepness (see~\autoref{fig:softmax})

\begin{equation}\label{eq:softmax}
    softmax(z_i) = \frac{\exp(\beta \cdot z_i)}{\sum_{k=0}^K{\exp(\beta \cdot z_k)}}
\end{equation}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \begin{axis}[samples=500,
            ...
            \node[anchor=west] (source) at (axis cs:2,0.6){};
            \node (destination) at (axis cs:0.5,0.9){$\beta$};
            \draw[thick, ->](source)--(destination);

            \addlegendentry{$softmax(x), \beta = 1$}
            \addlegendentry{$softmax(x), \beta = 2$}
            \addlegendentry{$softmax(x), \beta = 3$}
            \addlegendentry{$softmax(x), \beta = 4$}
            \addlegendentry{$softmax(x), \beta = 5$}
            \addlegendentry{$softmax(x), \beta = 6$}
        \end{axis}
    \end{tikzpicture}
    \caption{The extended softmax for different values of the hyperparameter
        $\beta$. The plots have been obtained considering a two classes setting
        where $z_1$ is always $1$. As $\beta$ increases, the function becomes
        steeper.\label{fig:softmax}}
\end{figure}
