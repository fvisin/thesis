...
from a neuron $h_i^{(l)}$ to a neuron $h_j^{(l)}$ is associated to a weight
$w_{ij}^{(l)}$.
...
the Perceptron -- called \emph{activation function}. One layer can thus be seen
as a function $g^{(l)}$ that depends on some \emph{parameters} -- namely the
weights matrix and bias of the layer, $\mathbf{W}^{(l)}$ and $b^{(l)}$ -- as
well as some \emph{hyperparameters} such as, e.g., the number of neurons and
the choice of activation function. It is common to refer to the trainable
parameters that fully characterize a layer as \emph{sufficient statistics},
usually denoted as $\mathbf{\theta}$.
...
\begin{equation}\label{eq:fn_composition}
    \textbf{y} = g^{(L)}_{\boldsymbol{\theta}} \circ g^{(L-1)}_{\boldsymbol{\theta}} \circ
    \dots \circ g^{(1)}_{\boldsymbol{\theta}}
\end{equation}


The MLP is one of the most used architectures for ANNs, yet not the only one.
In \autoref{sec:cnn} and \autoref{sec:rnn} two of the most used alternatives
will be described. In general every layer of an ANN will compute some
activation based on its input and a nonlinearity, that is usually different
from the one proposed in the Perceptron paper. In the following section, some
of the most used activation functions will be described.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[domain=-2.9:2.9]
        ...
        % axes
        \draw[->,thick] (-2.9, 0) -- (3.1, 0) node[right] {$a$};
        \draw[->,thick] (0, -1.1) -- (0, 3.1) node[above] {$f(a)$};

        % Logistic
        \draw[thick, color=blue]
            plot (\x, { 1/(1+exp(-1 * \x)) })
            node[right,yshift=-0.5em] {$logistic(a)$};
        % Beta logistic
        ...
        \draw[thick, color=orange]
            ...
            node[right,yshift=0.5em] {$tanh(a)$};
        % ReLU
        \draw[thick, color=red]
            plot (\x, { max(0, \x) })
            %node[right] {$ReLU(x)$};
            node[xshift=-19.3em,yshift=-8em] (a) {$ReLU(a)$};
        % Leaky ReLU
        \draw[thick, color=yellow]
            plot (\x, { max(0.03 * \x, \x) })
            node[below = 1.2em of a.east, anchor=east] {$Leaky ReLU(a)$};
    \end{tikzpicture}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \begin{axis}[samples=50,
            ...
            \addlegendentry{$softmax(a), T= 3$}
            \addlegendentry{$softmax(a), T= 2$}
            \addlegendentry{$softmax(a), T= 1$}
            \addlegendentry{$softmax(a), T= 1/2$}
            \addlegendentry{$softmax(a), T= 1/3$}
            \addlegendentry{$softmax(a), T= 1/4$}
            \addlegendentry{$softmax(a), T= 1/5$}
            \addlegendentry{$softmax(a), T= 1/6$}
        \end{axis}
    \end{tikzpicture}
    \caption{The behaviour of softmax as temperature $T$ grows. The plots have
        been obtained considering a bidimensional input setting where the value
        associated to the second class $a_1$ is always $1$. As $T$ increases,
        the function becomes steeper.\label{fig:softmax}}
\end{figure}
...
once again~\autoref{fig:MLP}: the network processes a bidimensional input and,
after three layers of affine transformations followed by a nonlinearity,
outputs a unidimensional value $\mathbf{y}$. The correct output $\mathbf{\hat
y}$ is given and is used to compute the error, or~\emph{cost}, with some
metric. Consider e.g. the mean squared error (MSE)

\begin{equation}\label{eq:MSE}
    E_{MSE} = \frac{1}{M} \sum_{s=0}^M(\mathbf{\hat y}^{(s)} - \mathbf{y}^{(s)})^2
\end{equation}

\noindent where $M$ is the overall number of samples in the dataset and the
superscript in $\mathbf{y}^{(s)}$ is used to represent one sample of the
dataset (rather than the id of the layer, since there is only one output
layer).

It is possible to compute the fraction of the error that is imputable to one of
the neurons of the last layer, $w_{ij}^{(4)}$, by taking the derivative of the
error w.r.t. to it:

\begin{align*}\label{eq:backprop_step1}
    \frac{\partial E_{MSE}}{\partial w_{ij}^{(4)}} &=
        \frac{\partial}{\partial w_{ij}^{(4)}}(\frac{1}{M}\sum_{s=0}^M
        (\mathbf{\hat y}^{(s)} - \mathbf{y}^{(s)})^2) \\
    &= \frac{1}{M}\sum_{s=0}^M \frac{\partial}{\partial w_{ij}^{(4)}}(
        (\mathbf{\hat y}^{(s)} - \mathbf{y}^{(s)})^2) \\
    &= \frac{1}{M}\sum_{s=0}^M \frac{\partial}{\partial w_{ij}^{(4)}}(
        (\mathbf{\hat y}^{(s)} - \mathbf{y}^{(s)})^2) \\
\end{align*}
