...
%     % step function
%     \draw[line width=0.05mm]([xshift=0pt]input)  +(0,-12pt) -- +(0,12pt); % thin vert
%     \draw[line width=0.25mm](input)  +(0pt,8pt) -- +(12pt,8pt); %top horiz
%     \draw[line width=0.25mm]([xshift=0pt]input)  +(0,-8pt) -- +(0,8pt); % wide vert
%     %\draw[rotate=90]([xshift=-9pt]input)  +(0,-12pt) -- +(0,12pt);
%     \draw[line width=0.25mm](input)  +(-12pt,-8pt) -- +(0pt,-8pt); %top horiz
% \end{tikzpicture}
...
\begin{figure}[h]
    ...
    \begin{neuralnetwork} [nodespacing=6mm, layerspacing=25mm,
        ...
        \newcommand{\nodetextstep}[2]{
            ...
            \draw[line width=0.05mm](0,0)  +(0,-8pt) -- +(0,8pt); % thin vert
            \draw[line width=0.25mm](0,0)  +(0pt,5pt) -- +(8pt,5pt); % wide top horiz
            \draw[line width=0.25mm](0,0)  +(0,-5pt) -- +(0,5pt); % wide vert
            \draw[line width=0.25mm](0,0)  +(-8pt,-5pt) -- +(0pt,-5pt); % wide top horiz
            \end{tikzpicture}
        }

        % Layers
        \inputlayer[count=5, bias=false, title=Input\\layer, text=\nodetextxi]   % bias=true, nodeclass={input neuron}
        \outputlayer[count=1, text=\nodetexts]   % bias=false, nodeclass={output neuron}
...
The activation rule of the perceptron is very simple and is at the base\todo{verifica base or bases} of
...
of the Perceptron and $x_i^t$ and $w_i^t$ are respectively the $i$-th input and
weight at time $t$.
...
The excessive excitement for the early successes of ANNs turned into strong
...
interact with humans appeared suddently unrealistic and most of the research
community lost interest in ANNs.

The field experienced a severe slow down after most of the fundings were cut,
until in 1982 John Hopfield presented a model of the human memory that did not
only give insights on how the brain works, but was also useful in practical
applications and had a sound and detail mathematical grounding. At the same
time at the US-Japan Joint Conference on Cooperative/Competitive Neural
Networks Japan announched a renewed effort in building Neural Networks and the
fear that the US might be left behind renewed the US effort on this topic.

The real breakthrough that caused the

It was nt until the rediscovery of the backpropagation algorithm in 1986 that the field gained widespread interest again.
...
It wasn't until 1986, when the backpropagation algorithm~\cite{origBackprop}
that allowed to train ANNs composed by more than one layer was
rediscovered~\citep{backprop}, that ANNs became an active field of study again.
Since then, ANNs have been applied to many fields, achieving state of the art
results in computer vision~\cite{something}, robotics~\cite{something},
medical diagnostics~\cite{something}, astronomy~\cite{something}, ..
