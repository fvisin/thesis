...
%the neurons of the last layer, i.e. to the weights $\mathbf{W}^{(4)}$,
by taking the derivative of the error w.r.t. to its weights
%. The notation $\nabla^{(4)}$
%will be used to denote the gradient of the error w.r.t. $\mathbf{W}^{(4)}$.
...
\begin{equation}\label{eq:backprop_step2}
    \frac{\partial }{\partial w_{ij}^{(2)}}(-\mathbf{y}) =
        ...
        \frac{\partial \mathbf{h}^{(3)}}{\partial w_{ij}^{(2)}} \\
\end{equation}
    %&= \frac{1}{M}\sum_{s=0}^M \frac{\partial}{\partial \mathbf{W}^{(4)}}
    %    (\mathbf{\hat y}^{(s)} - \mathbf{W}^{(4)} \cdot \mathbf{h}^{(3)})^2 \\
    % &= \frac{1}{M}\sum_{s=0}^M \frac{\partial}{\partial \mathbf{W}^{(4)}}(
    %     (\mathbf{\hat y}^{(s)} - \mathbf{W}^{(4)} \cdot h_i^{(3)})^2) \\
    % &= \frac{1}{M}\sum_{s=0}^M
    %     2(\mathbf{\hat y}^{(s)} - \mathbf{W}^{(4)} \cdot h_i^{(3)})^2) -
    %     h_i^{(3)} \\
